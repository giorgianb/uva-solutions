...
size_t block_position (const int block_pile[], const int block);

void
clear_block_pile (int blocks[MAX_NBLOCKS][MAX_NBLOCKS + 1],
		  size_t location[MAX_NBLOCKS], const int bottom_block);
...
main (void)
{
  ...
  if (scanf ("%zu", &nblocks) != 1)
    {
      perror (__LOC__ ": Could not read nblocks");
      exit (EXIT_FAILURE);
    }
  ...
  int nread;
  while ((nread =
	  scanf ("%5s %zu %5s %zu", command, &block1, directive,
		 &block2)) == 4)
    {
      int cmd;
	...
	{
	  fprintf (stderr, __LOC_PRE__ "Unrecognized command: %s\n", command);
	  exit (EXIT_FAILURE);
	}
      ...
      if (cmd == QUIT_COMMAND_NUM)
	{
	  fprintf (stderr,
		   __LOC_PRE__
		   "Unexpected parameters for 'quit' command: %zu %s %zu\n",
	...
	{
	  fprintf (stderr, __LOC_PRE__ "Unrecognized directive: %s\n",
		   directive);
	  exit (EXIT_FAILURE);
	}
	...
	{
	  fprintf (stderr,
		   __LOC_PRE__ "Invalid block specification: %zu %zu\n",
		   block1, block2);
	  exit (EXIT_FAILURE);
	}
	...
	case (MOVE_COMMAND_NUM | OVER_DIRECTIVE_NUM):
	  {
	    ...
	    blocks[location[block1]][block_position
				     (blocks[location[block1]], block1)] =
	      NO_BLOCK;
	...
	case (PILE_COMMAND_NUM | OVER_DIRECTIVE_NUM):
	  {
	    ...
	    const size_t b1_pile_begin =
	      block_position (blocks[old_pile], block1);
	    size_t top = block_position (blocks[location[block2]], NO_BLOCK);

	    for (size_t i = b1_pile_begin;
		 blocks[old_pile][i] != NO_BLOCK && i < 2 * MAX_NBLOCKS; ++i)
	      {
	...
	default:
	  fprintf (stderr, __LOC_PRE__ "Unexpected command: %d\n", cmd);
	  exit (EXIT_FAILURE);
	}
    }
  ...
  if (nread != 1 && strcmp (command, QUIT_COMMAND) == 0)
    {
      perror (__LOC__ ": Error reading command");
      exit (EXIT_FAILURE);
    }
...
{
  size_t pos;
  for (pos = 0; block_pile[pos] != NO_BLOCK && block_pile[pos] != block;
       ++pos)
    ;
